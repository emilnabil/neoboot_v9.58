# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/NeoBoot/plugin.py
...
if not fileExists('/etc/vtiversion.info') and not fileExists('/etc/bhversion') and fileExists('/usr/lib/python2.7') or fileExists('/usr/lib/python3.9') and fileExists('/usr/lib/python3.10') and fileExists('/.multinfo'):
...
UPDATEVERSION = '9.58'
PASSWORD = '1234'
VIPVERSION = 'NEOBOOT VIP VERSION'
UPDATEDATE = '"+%Y11%d"'
...
class NeoBootInstallation(Screen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['WizardActions',
         ...
         'NumberActionMap',
         'SetupActions',
         'numberDirectionActions'], {'red': self.Instrukcja,
    ...
    def first_installationNeoBoot(self):
        ...
        out.close()
        if os.path.isfile('%sImageBoot/.neonextboot' % getNeoLocation()):
            os.system('rm -f /etc/neoimage; rm -f /etc/imageboot; rm -f %sImageBoot/.neonextboot; rm -f %sImageBoot/.version; rm -f %sImageBoot/.Flash; rm -f %sImageBoot/.imagedistro; rm -f %sImageBoot/.initneo.log; rm -f %sImageBoot/.updateversion' % (getNeoLocation(),
             getNeoLocation(),
             getNeoLocation(),
             getNeoLocation(),
             getNeoLocation(),
        ...
        if getCPUtype() == 'ARMv7':
            os.system('cd ' + LinkNeoBoot + '/;chmod 755 ./files/findsk.sh; mv ./bin/fbcleararm ./bin/fbclear; chmod 755 ./bin/fbclear; rm -f ./bin/nandwrite; rm -f ./bin/fbclearmips; mv ./ubi_reader_arm ./ubi_reader; rm -r ./ubi_reader_mips; rm ./bin/neoinitmips; rm ./bin/neoinitmipsvu; rm -r ./bin/nanddump_mips; rm ./bin/nfidump; rm ./bin/neobmmips; mv ./bin/neobmarm ./bin/neobm; rm ./bin/fontforneoboot.ttf; rm ./bin/libpngneo; cd')
        elif getCPUtype() == 'MIPS':
...
class NeoBootImageChoose(Screen):
    ...
    def DownloadImageOnline(self):
        if not os.path.exists('/usr/lib/enigma2/python/Plugins/Extensions/ImageDownloader/download.py'):
            message = _('Plugin ImageDownloader not installed!\nInstall plugin to download new image? \x07nd---Continue ?---')
        ...
        if yesno:
            if checkInternet():
                ...
                if fileExists('/usr/lib/enigma2/python/Plugins/Extensions/ImageDownloader/plugin.py'):
                    ...
                    self.close()
                elif not fileExists('/usr/lib/enigma2/python/Plugins/Extensions/ImageDownloader/plugin.py'):
    ...
    def touch4(self):
        if fileExists('/usr/lib/periodon/.kodn'):
            pass
        elif not fileExists('/tmp/gutosi'):
            pass
        elif not fileExists('/usr/lib/periodon'):
            system('mkdir /usr/lib/periodon')
        elif getButtonPin() == 'pinok':
    ...
    def touch9(self):
        ...
        elif not fileExists('/tmp/gutosie'):
            pass
        else:
    ...
    def touch8(self):
        ...
        elif not fileExists('/tmp/gutosiep'):
            pass
        else:
    ...
    def touch0(self):
        if fileExists('/usr/lib/periodon/.kodn'):
            if not fileExists('/tmp/gut3'):
                ...
                restartbox.setTitle(_('Restart GUI now !!'))
        elif not fileExists('/tmp/gutosiepi'):
            pass
        else:
    ...
    def RemoveIMAGE(self, yesno):
        if yesno:
            ...
            self.session.openWithCallback(self.up, Console, _('NeoBoot: Deleting Image'), [cmd,
             cmd1,
             cmd2,
             cmd3,
             cmd5])
    ...
    def ImageInstall(self):
        if not fileExists('/.multinfo'):
            ...
            elif fileExists('/usr/lib/periodon/.activatedmac'):
                self.ImageInstallTestOK()
            else:
                myerror = _('Sorry, this is not neoboot vip version.\nGet NEO-VIP version, more info press blue button or try to update.')
                self.session.open(MessageBox, myerror, MessageBox.TYPE_INFO)
    ...
    def ImageInstallTestOK(self):
        if int(self.availablespace) < 500:
            myerror = _('Not enough free space on /media/ !!\nYou need at least 500Mb free space.\n\nExit plugin.')
            self.session.open(MessageBox, myerror, MessageBox.TYPE_INFO)
        elif getSupportedTuners() == getBoxHostName():
            try:
                self.GOImageInstall()
            except Exception as e:
                loggscrash = time.localtime(time.time())
...
def main(session, **kwargs):
    ...
    if vip == 1:
        ...
        if not fileExists('/.multinfo') and fileExists('' + LinkNeoBoot + '/.location'):
            ...
            if not fileExists('/usr/lib/periodon/.kodn'):
                system('touch /usr/lib/periodon/.kodn')
                os.system(_('echo %s  > /usr/lib/periodon/.kodn') % PASSWORD)
                os.chmod("/usr/lib/periodon/.kodn", 0o644) 
            if not fileExists('/usr/lib/periodon/.accessdate'):
                os.system('date %s  > /usr/lib/periodon/.accessdate' % UPDATEDATE)                                                            
            if not fileExists('/tmp/.nkod'):
                system('touch /tmp/.nkod')
                os.system(_('echo %s  > /tmp/.nkod') % UPDATEVERSION)
                os.chmod("/tmp/.nkod", 0o644)
            if not fileExists('/usr/lib/periodon/.activatedmac'):
                system('touch /usr/lib/periodon/.activatedmac')
                os.system("ifconfig eth0 |grep -i hwaddr |awk '{print $5}' > /usr/lib/periodon/.activatedmac")
                os.chmod("/usr/lib/periodon/.activatedmac", 0o644)
            if not fileExists('/usr/lib/periodon/.fullaccess'):
                system('touch /usr/lib/periodon/.fullaccess')
                os.system(_('echo %s  > /usr/lib/periodon/.fullaccess') % VIPVERSION)
                os.chmod("/usr/lib/periodon/.fullaccess", 0o644)                                        
